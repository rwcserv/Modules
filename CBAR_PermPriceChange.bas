Attribute VB_Name = "CBAR_PermPriceChange"
Option Explicit
...
Sub CBAR_PermPriceChange(Optional ByVal BuyerEmailers As Boolean)
    ...
    If CBA_BasicFunctions.isRunningSheetDisplayed = False Then CBA_BasicFunctions.CBA_Running "Preparing to run 'Permanent Price Change Report'"
    ...
    If isThereData = True Then
        ...
        For a = LBound(CBA_COM_Match) To UBound(CBA_COM_Match)
            If CBA_COM_Match(a).Pricedata(Dto, "Special", tR.State, False) = False Then
                If CBA_COM_Match(a).Pricedata(Dto, "Shelf", tR.State, False) <> CBA_COM_Match(a).Pricedata(DFrom, "Shelf", tR.State, False) And _
                    CBA_COM_Match(a).Pricedata(Dto, "Shelf", tR.State, False) <> 0 And CBA_COM_Match(a).Pricedata(DFrom, "Shelf", tR.State, False) <> 0 And _
                    ((CBA_COM_Match(a).Pricedata(DFrom, "Shelf", tR.State, False) + CBA_COM_Match(a).Pricedata(DFrom, "Discount", tR.State, False)) <> _
                    (CBA_COM_Match(a).Pricedata(Dto, "Shelf", tR.State, False) + CBA_COM_Match(a).Pricedata(Dto, "Discount", tR.State, False))) Then
        ...
        If dirMatch.Count > 0 Then
            ...
            wks_OP.Name = "PermPriceChange"
            If BuyerEmailers = True Then CBAR_ReportParamaters.setBuyerEmailerWorksheet "CBAR_PermPriceChange", wks_OP
            ...
            With wks_OP
                ...
                .Cells(5, 5).Value = "States of Product Activity"
                .Cells(5, 6).Value = "Old Shelf Retail"
                .Cells(5, 7).Value = "Price Change"
                .Cells(5, 8).Value = "New Shelf Retail"
                .Cells(5, 9).Value = "New ProRata Retail"
                .Cells(5, 10).Value = "Aldi Retail"
                .Cells(5, 11).Value = "Old Aldi Diff %"
                .Cells(5, 12).Value = "New Aldi Diff %"
                ...
                For a = 1 To 20
                    Select Case a
                        ...
                        Case 6, 7, 8, 9, 10, 11, 12, 18, 19, 20
                    ...
                    End Select
                Next
                .Cells(3, 3).Value = "Permanent Price Change Report"
                For Each m In dirMatch
                        ...
                        .Cells(no, 6).Value = dirMatch(m).Pricedata(DFrom, "shelf", tR.State)
                        .Cells(no, 7).Value = dirMatch(m).Pricedata(Dto, "shelf", tR.State) - dirMatch(m).Pricedata(DFrom, "shelf", tR.State)
                        .Cells(no, 8).Value = dirMatch(m).Pricedata(Dto, "shelf", tR.State)
                        .Cells(no, 9).Value = dirMatch(m).Pricedata(Dto, "prorata", tR.State)
                        .Cells(no, 10).Value = dirMatch(m).Pricedata(Dto, "aldiretail", tR.State)
                        .Cells(no, 11).Value = dirMatch(m).Pricedata(DFrom, "delta", tR.State)
                        ...
                        .Cells(no, 12).Value = dirMatch(m).Pricedata(Dto, "delta", tR.State)
                        If .Cells(no, 12).Value <> 0 Then .Cells(no, 12).Interior.ColorIndex = CBA_COM_ReportFunctions.CBA_COM_isTrafficLight(.Cells(no, 12).Value, dirMatch(m).MatchType)
                        For b = 1 To 5
                            If b = 1 Then st = "NSW" Else If b = 2 Then st = "VIC" Else If b = 3 Then st = "QLD" Else If b = 4 Then st = "SA" Else If b = 5 Then st = "WA"
                            If dirMatch(m).Pricedata(Dto, "shelf", st) = dirMatch(m).Pricedata(Dto, "shelf", tR.State) Then
...
Err_Routine:
    CBA_Erl = CLng(VBA.Erl): Call CBA_ProcI("s-CBAR_PermPriceChange", 3)
