Attribute VB_Name = "CBAR_REPORTWBA"
Option Explicit
...
Sub WBA(ByVal StateLookup As String, ByVal DFrom As Date, ByVal Dto As Date, ByVal DisplayData As String, ByRef GraphsToMake As Collection, ByVal CGno As Long, ByVal OldM As Boolean)
    ...
    Dim isMatchDataAvaliable As Boolean
    ...
    If UBound(CBA_COM_Match) < 0 Then
        Err.Clear
        On Error GoTo Err_Routine
        isMatchDataAvaliable = CBA_COM_SetupMatchArray.CBA_SetupMatchArray(OldM, DFrom, Dto, CGno)
    Else
        isMatchDataAvaliable = True
        If MsgBox("Would you like to re-run the match objects?", vbYesNo) = 6 Then isMatchDataAvaliable = CBA_COM_SetupMatchArray.CBA_SetupMatchArray(OldM, DFrom, Dto, CGno)
    End If
    On Error GoTo Err_Routine
    If isMatchDataAvaliable = True Then
        ...
        ElseIf LCase(DisplayData) = "weeks" Then
            strSQL = strSQL & "select datepart(iso_week,posdate) as prd, sum(retail) as totbus into #Totbus from cbis599p.dbo.pos pos where posdate >= @datefrom and posdate <= @dateto group by datepart(iso_week,posdate)" & Chr(10)
            strSQL = strSQL & "select datepart(iso_week,posdate),isnull(p.con_productcode,p.productcode), round((sum(retail) / tb.totbus) * 100,2) from cbis599p.dbo.pos pos" & Chr(10)
            strSQL = strSQL & "left join cbis599p.dbo.product p on p.productcode = pos.productcode" & Chr(10) & "inner join #Prods pr on pr.productcode = isnull(p.con_productcode,p.productcode)" & Chr(10)
            strSQL = strSQL & "left join #totbus tb on tb.prd = datepart(iso_week,posdate)" & Chr(10)
            strSQL = strSQL & "where posdate >= @datefrom and posdate <= @dateto" & Chr(10)
            strSQL = strSQL & "group by  datepart(iso_week,posdate), isnull(p.con_productcode,p.productcode), tb.totbus  order by isnull(p.con_productcode,p.productcode) " & Chr(10)
        ...
        For Each dset In GraphsToMake
            ...
            For a = 1 To UBound(CBA_COM_Match) + 1
                ...
                If ThisProd <> CBA_COM_Match(v).AldiPCode Or a = UBound(CBA_COM_Match) + 1 Then
                    If a > 1 Then
                        For j = Month(DFrom) To Month(Dto)
                                ...
                                If LCase(retARR(2, b)) = LCase(StateLookup) Then
                                    If myprod <> CBA_COM_Match(a).AldiPCode Then
                                        ...
                                        For c = 0 To UBound(CBISarr, 2)
                                            ...
                                            End If
                                        Next
                                    ElseIf Month(retARR(1, b)) <= Month(Dto) Then
                                    ...
                                    If ForEntry = True Then
                                        If Aretarr(3, b) > 0 Then
                                            If retARR(1, b) >= DFrom And retARR(1, b) <= Dto Then
...
completereport:
            ...
            With wks_WBA
                ...
                'If lNum = 0 Then
                    lNum = lNum + 1
                    '.Cells(lNum, 10 + numfornextstart).Value = UBound(CBA_COM_Match) & " CBA_COM_Match Records"
            ...
            '        .Cells(22, 10).Value = Retails(a).ambientnonfood
            '        .Cells(23, 10).Value = AldiRetails(a).ambientnonfood
        ...
        'REMOVING ALCOHOL AND PRODUCE FROM DATA
        DoEvents
