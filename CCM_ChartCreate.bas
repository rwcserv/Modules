Attribute VB_Name = "CCM_ChartCreate"
Option Explicit
Option Private Module       ' Excel users cannot access procedures

Function CBA_COM_ChartCreate(ByRef cLeft As Single, ByRef cTop As Single, ByRef cSourceRng As Range, ByRef cWkst As Worksheet, _
    ...
    Dim cht As ChartObject, numGen As Long, NumSeries As Long, numCategory As Long
...
Err_Routine:
    CBA_Erl = CLng(VBA.Erl): Call CBA_ProcI("f-CBA_COM_ChartCreate", 3)
...
Function CBA_BTF_ChartCreate(ByRef cLeft As Single, ByRef cTop As Single, ByRef cSourceRng As Range, ByRef cWkst As Worksheet, _
                        Optional ByRef cTitle As String, _
                            Optional ByRef cLegendRng As Range, _
                                Optional ByRef cCategoryRng As Range, _
                                        Optional ByRef cLegPos, _
    ...
    Dim cht As ChartObject, numGen As Long, NumSeries As Long, numCategory As Long, dls As DataLabels, lIdx As Long
    On Error GoTo Err_Routine
    CBA_ErrTag = ""
    ...
    With cWkst
        ...
        'Apply Legend and Axis names and formatting
        If Not cLegendRng Is Nothing Then
            With cht.Chart
                For numGen = 1 To NumSeries
                    If numGen = 1 And Not IsMissing(cType) Then      ''''Or cLegendRng.Resize(1, 1).Value = "POS Quantity" Then
                        .SeriesCollection(numGen).Name = cLegendRng.Resize(1, 1).Offset(0, numGen - 1).Value
                        .HasAxis(xlCategory, xlPrimary) = True
                        .SeriesCollection(numGen).ChartType = cType
                        .HasAxis(xlValue, xlPrimary) = True
...
''                        .Axes(xlValue, xlPrimary).ReversePlotOrder = True
                    ElseIf numGen = 2 And Not IsMissing(cSecType) Then
                        .SeriesCollection(numGen).ChartType = cSecType
                        .SeriesCollection(numGen).AxisGroup = xlSecondary
                        '.HasAxis(xlCategory, xlSecondary) = False
                        .HasAxis(xlValue, xlSecondary) = True
                        ...
                        If cAxisTit = True Then
                            ...
                            .Axes(xlValue, xlSecondary).AxisTitle.text = cLegendRng.Resize(1, 1).Offset(0, numGen - 1).Value
                        End If
''                        Set dls = .SeriesCollection(1).DataLabels
''                        For lIdx = 1 To dls.Count
''                            dls(lIdx).font.Color = RGB(255, 0, 0)
''                        Next
''                        .Axes(xlValue, xlSecondary).ReversePlotOrder = True
''                        .PlotArea.Format.Fill.ForeColor.RGB = RGB(255, 0, 0)
                    ElseIf numGen > 1 And IsMissing(cSecType) Then
                        .SeriesCollection(numGen).Name = cLegendRng.Resize(1, 1).Offset(0, numGen - 1).Value
                        .HasAxis(xlCategory, xlPrimary) = True
                        .SeriesCollection(numGen).ChartType = cType
                        .HasAxis(xlValue, xlPrimary) = True
        ...
        If Not cCategoryRng Is Nothing Then
            numCategory = cCategoryRng.Cells.Count
            cht.Chart.Axes(xlCategory, xlPrimary).CategoryNames = cCategoryRng
''            cht.Chart.Axes(xlCategory, xlPrimary).ReversePlotOrder = True
''            cht.Chart.Axes(xlValue, xlPrimary).ReversePlotOrder = True
''''            cht.Chart.Axes(xlCategory).ReversePlotOrder = False
''''            cht.Chart.Axes(3).ReversePlotOrder = True
''            cht.Chart.Axes(xlValue, xlSecondary).ReversePlotOrder = False
''
''            cht.Chart.Axes(xlValue, xlSecondary).ReversePlotOrder = True
''            cht.Chart.Axes(xlCategory, xlPrimary).ReversePlotOrder = True
        End If
...
Err_Routine:
    CBA_Erl = CLng(VBA.Erl): Call CBA_ProcI("f_CBA_BTF_ChartCreate", 3)
    CBA_Error = " Error - " & Err.Number & "-" & Err.Description & "-" & CBA_ProcI(, 0) & "-" & CBA_ErrTag
'    If CBA_ErrTag = "SQL" Then CBA_Error = CBA_Error & vbCrLf & CBA_strSQL_TBLNAME
    Debug.Print CBA_Error
    Call g_FileWrite(g_GetDB("Gen", True), CBA_Error, , , True, True)
