Attribute VB_Name = "CBAR_NoLongerPromo"
'Option Explicit
...
Sub CBAR_NoLongerOnPromo(Optional ByVal BuyerEmailers As Boolean)
    ...
    Dim BDDic As Variant, BADic As Variant
    Dim BDBADic As Scripting.Dictionary
    Dim tR As CBAR_Report
    Dim isThereData As Boolean
    ...
    CS = "": WS = "": DS = "": FS = "": AMS = ""
        ...
        If CBA_BasicFunctions.isRunningSheetDisplayed = False Then CBA_BasicFunctions.CBA_Running "Preparing to run 'Off Promo Report'"
    ...
    'If tR.BD = "Produce" Or tR.CG = 58 Then Else
    ...
    Else
        Dto = CBA_COM_Runtime.CBA_getWedDate(DateAdd("D", 1 - WeekDay(Date, 4), Date))
        scraped = CBAR_ReportParamaters.getEmailerScrapedDatesArray
        dates = UBound(scraped)
        DFrom = DateAdd("WW", -4, Dto)
    End If
    ...
    If BuyerEmailers = False Then isThereData = CBA_COM_SetupMatchArray.CBA_SetupMatchArray(True, DFrom, Date, tR.CG, tR.scg, strProds, True, tR.Competitor) Else isThereData = True
    If isThereData = True Then
        ...
        For a = LBound(CBA_COM_Match) To UBound(CBA_COM_Match)
            If (CBA_COM_Match(a).AldiPCG = 58 And CBA_COM_Match(a).Pricedata(DateAdd("D", -1, Date), "Special", tR.State, False) = True) Or _
                (CBA_COM_Match(a).AldiPCG <> 58 And CBA_COM_Match(a).Pricedata(scraped(UBound(scraped) - 1), "Special", tR.State, False) = True) Then
                If (CBA_COM_Match(a).AldiPCG <> 58 And CBA_COM_Match(a).Pricedata(Dto, "Special", tR.State, False) = False) Or (CBA_COM_Match(a).AldiPCG = 58 And CBA_COM_Match(a).Pricedata(Date, "Special", tR.State, False) = False) Then
        ...
        If dirMatch.Count > 0 Then
            ...
            wks_OP.Name = "NoLongerOnPromo"
            If BuyerEmailers = True Then CBAR_ReportParamaters.setBuyerEmailerWorksheet "CBAR_NoLongerPromo", wks_OP
            With wks_OP
                ...
                .Cells(5, 5).Value = "Most Recent State of Promotion"
                .Cells(5, 6).Value = "Promo Retail"
                .Cells(5, 7).Value = "Price Saving"
                .Cells(5, 8).Value = "Return Shelf Retail"
                .Cells(5, 9).Value = "Return ProRata Retail"
                ...
                For a = 1 To 20
                    ...
                    End Select
                Next
                .Cells(3, 3).Value = "Comp Products no longer on Promotion"
                For Each m In dirMatch
                        ...
                        .Cells(no, 6).Value = dirMatch(m).Pricedata(DFrom, "prorata", tR.State)
                        .Cells(no, 7).Value = dirMatch(m).Pricedata(DFrom, "discount", tR.State)
                        ...
                        For a = UBound(scraped) - 1 To LBound(scraped) Step -1
                            If dirMatch(m).Pricedata(scraped(a), "special", tR.State) = True Then
                                ...
                                If .Cells(no, 5).Value = "" Then
                                    For b = 1 To 5
                                        ...
                                        If dirMatch(m).Pricedata(scraped(a), "special", st) = True Then
                                            If .Cells(no, 5).Value = "" Then
                                                .Cells(no, 5).Value = st
                                            Else
                            ...
                            End If
                        Next
                        If Scnt = weeks Then .Cells(no, 12).Value = weeks & "+" Else .Cells(no, 12).Value = Scnt
...
Err_Routine:
    CBA_Erl = CLng(VBA.Erl): Call CBA_ProcI("s-CBAR_NoLongerOnPromo", 3)
